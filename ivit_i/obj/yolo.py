...
class YOLO(Model):
    class Params:
        ...
        def __init__(self, param, sides, labels, anchors):
            ...
            if not anchors:
                ...
                self.anchors = param.get('anchors', anchors)
            self.isYoloV3 = False
    ...
    def __init__(self, ie, model_path, labels=None, keep_aspect_ratio=False, threshold=0.5, iou_threshold=0.5,anchors=None):
    ...
    def _get_output_info(self):
        ...
        for node in ng_func.get_ordered_ops():
            ...
            yolo_params = self.Params(node._get_attributes(), shape[2:4], self.labels, self.anchors)
...
class YoloV4(YOLO):
    ...
    def __init__(self, ie, model_path, labels=None, keep_aspect_ratio=False, threshold=0.5, iou_threshold=0.5,
        ...
        super().__init__(ie, model_path, labels, keep_aspect_ratio, threshold, iou_threshold, anchors)
    ...
    def _parse_yolo_region(predictions, input_size, params, threshold, multiple_labels=True):
        ...
        for row, col, n in np.ndindex(params.sides[0], params.sides[1], params.num): 
            ...
            # print("x:{}, y:{}, height:{}, width:{}, box_score:{}, {}".format(x, y, height,width,object_probability,class_probabilities))
            if object_probability < threshold:
                continue
            # Process raw value
            x = (col + x) / params.sides[1]
